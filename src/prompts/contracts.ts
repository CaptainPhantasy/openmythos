...
Each task must include id, title, description, role, fileTargets, acceptanceCriteria.
tasks must be a JSON array. dependencies must be a JSON object mapping task ids to arrays of dependency task ids.
successCriteria, fileTargets, and acceptanceCriteria must be JSON arrays of strings.
...
Do not output diffs. Do not output TODO placeholders. Do not modify files unrelated to the task.`;
...
Only include fileEdits when you are providing complete corrected file content.
